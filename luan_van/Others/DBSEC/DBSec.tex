...
\begin{document}
	...
	%\author{Anonymous}% -*- program: Value -*-
	\author{Luong Khiem Tran\inst{1} \and Ha Xuan Son\inst{1, 2} \and Tran Khanh Dang\inst{1} \and Kim Tuyen Le\inst{1}}
	%
	\authorrunning{Luong Khiem Tran et al.} %Sabbreviated author list (for running head)
	%
	%%%% list of authors for the TOC (use if author list has to be modified)
	\tocauthor{Luong Khiem Tran, Ha Xuan Son, Dang Tran Khanh, and Le Thi Kim Tuyen}
	%
	\institute{Ho Chi Minh City University of Technology, Ho Chi Minh city, Vietnam,\\
		\email{khiem111189@gmail.com}, \email{hxson@ctuet.edu.vn}, \email{khanh@hcmut.edu.vn}, \email{tuyenltk@hcmut.edu.vn}\and
...
By this approach, administrators and patients are allowed to modify (insert, delete and edit) their policies in the real-time.% without any constraints on the part of the system. 
...
%The paper is organized as follows: the next section discusses relevant background information; followed by a proposed model and implementation.
%The proposed implementation is evaluated based on security analysis compare with the other models and measuring performance.
%The paper concludes with a summary of results and discussion for further research.
...
%This section gives an overview of the policies that are included in the system.
...
%We will then discuss the corresponding steps in the implementation of the policy update.
%The corresponding steps in the implementation of the policy update will be discussed in the following part.
...
%Each policy will have certain attributes that need protecting. 
Each policy will have certain attributes that need to be protected. 
%For example, the name of the patients will not be allowed to see excepting the doctors and nurses in charge of the patients' medical care. 
...
%The attribute that needs being secure is the patient's name, with the exception of the physicians and the nurses taking care of the patients, no one is allowed to access the patient's name attribute values. 
%According to Turkmet et al defined: policy space denotes the set of all possible requests \cite{turkmen2017formal}.
%In order to monitor the updating of policies in the system, a nicer policy management mechanism is needed, where each individual policy value (security attributes, constraint values) is managed. 
...
%In traditional approach is that all policies in the system are stored in a fixed place, and are called when necessary. 
...
Applicability Space is represented as a triple $\langle AS, NAS, IS \rangle$, in which  $AS$, $NAS$ contains the access requests for which the target and condition evaluates to \texttt{Match} (True) and \texttt{NoMatch} (False) respectively, and $IS$ contains the requests for %which evaluates the constraints produces an error.
...
The following section presents how a policy defined in XML/JSON is converted  to a combination of $at$ and $ac$. %as defined in definition 1.
...
\begin{figure}[ht!]
	\label{fig.condition}
	\centering
	\includegraphics[scale=0.35]{fig/Mainmodel.png}
...
The characteristics of these models are shown in Table 2.
...
\begin{minipage}{.5\textwidth}
	\centering
	\includegraphics[width=1\textwidth,left]{fig/attributes.png}
...
\begin{minipage}{.4\textwidth}
	\centering
	\includegraphics[width=1\textwidth,right]{fig/actions.png}
	\caption{Profiled Actions}
	\label{fig:actions}
\end{minipage}
\end{figure}
