...
public class RBACInstantiations {
	...
	public static RBAC generateRBACForCARDTest(boolean distributePerms){
		...
		HashMap<Integer, RBACMERConstraint[]> cardConsts = new HashMap<Integer, RBACMERConstraint[]>();
		RBACMERConstraint card1 = new RBACMERConstraint("Card1", new RBACRole[]{r1}, 3, RBACMERConstraint.ACTIVATION_CONST);
		cardConsts.put(RBACMERConstraint.ACTIVATION_CONST, new RBACMERConstraint[]{card1});
	...
	public static RBAC generateRBACForSoDTest(boolean distributePerms){
		...
		RBACUser[] allUsers = new RBACUser[]{alice, john, seth };
		RSoDConstraint[] rsodcons = new RSoDConstraint[]{new RSoDConstraint("sod1",new RBACRole[]{r1,r2, r3,r4}, 3, RBACConstraint.H_RSoD_CONST/*.D_RSoD_CONST*/)};
		HashMap<Integer, RSoDConstraint[]> rsodConsts = new HashMap<Integer, RSoDConstraint[]>();
		rsodConsts.put(RBACConstraint.H_RSoD_CONST/*.D_RSoD_CONST*/, rsodcons);
		RBAC system = new RBAC(allUsers, 
							   new RBACRole[]{r1, r2, r3, r4, r5},
							   permissions, 
							   consts,
		...
		RBACState state4_prime = new RBACState("state4_prime",state4.createDeepCloneForU2S(state4.getSetOfSessions()));
		sessionChanged = state4_prime.getSetOfSessions().get(system.findUser("seth")).get(0);
		sessionChanged.dropActiveRole(r1, null/*system.rSODConstraints.get(RBACMERConstraint.H_RSoD_CONST)*/);
		system.addPastState(state4_prime, sessionChanged);
		//system.print(false);
		waitForInput();
		
		RBACState state5 = new RBACState("state5",state4_prime.createDeepCloneForU2S(state4_prime.getSetOfSessions()));
