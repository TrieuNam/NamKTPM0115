...
/*	public void setSessionCombinations(RBACUser[] allUsers, HashMap<RBACUser, Vector<RBACSession>> user2sessions){
	 ...
	 * */
	public void computeUserCombinations(Integer[] usersUpToNow, Integer[] users, HashMap<RBACUser, 
										Vector<RBACSession>> user2sessions, int k, RBACUser[] actualUsers) {
		if (k == 0){
			System.out.print("{");
	...
	public String print(){
		...
		str += "}," + cardinality + "):" + (type == RBACConstraint.D_RSoD_CONST ? "DRSoD" : "HRSoD");
		System.out.println("}," + cardinality + "):" + (type == RBACConstraint.D_RSoD_CONST ? "DRSoD" : "HRSoD"));
		return str;
	}
