...
class RBACSession extends RBACElement{
	...
	public boolean dropActiveRole(RBACRole r, RBACConstraint[] multiSessionConstraintsNonHistoryBased){
		handleRoleDropForConstraints(multiSessionConstraintsNonHistoryBased);
		if (activeRoles != null){
			for (Iterator<RBACRole> it = activeRoles.iterator(); it.hasNext();){
				RBACRole role = it.next();
				if (role.getID().equals(r.getID())) {
	...
	public void dropAllActiveRoles(RBACConstraint[] multiSessionConstraintsNonHistoryBased){
		handleRoleDropForConstraints(multiSessionConstraintsNonHistoryBased);		
